...
class TestCoordinates(unittest.TestCase):
    def setUp(self):
        self.default = Coordinates(96, 102, 150, 228, 228, 80)
    ...
    def test_constructor(self):
        ...
        c = Coordinates(1.00000000005, 2.00000000004, -3.5000000009, 0.0, 0.0, 0.0)
    ...
    def test_zero(self):
        c = Coordinates(0, 0, 0, 0, 0, 0)
        self.assertTrue(c.is_zero())

    def test_aabb(self):
        # aabb - Axis Aligned Bounded Box.
        # original method checks if point belongs aabb.
        p1 = Coordinates(0, 0, 0, 0, 0, 0)
        p2 = Coordinates(2, 2, 2, 0, 0, 2)    # spaeter als neue Funktion hinzufuegen um Drehwinkel zu checken
        c = Coordinates(1, 1, 1, 0, 0, 1)
        self.assertTrue(c.is_in_aabb(p1, p2))
        self.assertTrue(c.is_in_aabb(p2, p1))
        c = Coordinates(0, 0, 0, 0, 0, 0)
        self.assertTrue(c.is_in_aabb(p1, p2))
        c = Coordinates(2, 2, 2, 0, 0, 2)
        self.assertTrue(c.is_in_aabb(p1, p2))
        c = Coordinates(2, 3, 2, 0, 0, 2)
        self.assertFalse(c.is_in_aabb(p1, p2))
        c = Coordinates(-1, 1, 1, 0, 0, 1)
        self.assertFalse(c.is_in_aabb(p1, p2))
        c = Coordinates(1, 1, 3, 0, 0, 1)
        self.assertFalse(c.is_in_aabb(p1, p2))
    ...
    def test_add(self):
        r = self.default + Coordinates(1, 2, 3, 4, 5, 6)
